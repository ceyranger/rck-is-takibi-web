...
- 2026-06-10: Tadilat Takibi merged district visual regression fixed. The screen now draws one district block per district group while preserving existing cell templates/commands. Full tests passed; live exe update was delayed until the app closed, and the user's newly saved `Data\tasks.db` / `Data\last-save.json` hashes stayed unchanged after publish.
