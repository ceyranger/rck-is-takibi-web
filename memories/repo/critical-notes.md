...
- 2026-06-10: Tadilat Takibi scroll regression fixed by returning the screen to the single virtualized `DisplayRows` list. The district label is visible on each row to avoid blank district cells; do not reintroduce nested district row controls for this screen. Full tests passed, live exe was updated after the app closed, and `Data\tasks.db` / `Data\last-save.json` hashes stayed unchanged.
