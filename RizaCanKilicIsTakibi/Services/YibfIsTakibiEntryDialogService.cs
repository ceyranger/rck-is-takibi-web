...
public sealed class YibfIsTakibiEntryDialogService : IYibfIsTakibiEntryDialogService
{
    private readonly IProjectCatalogUiState _catalogUiState;
    private readonly IProjectCatalogService _catalogService;

    public YibfIsTakibiEntryDialogService(
    ...
    public Task<YibfIsTakibiEntry?> ShowDialogAsync(CancellationToken cancellationToken = default)
    {
        var viewModel = new YibfIsTakibiEntryDialogViewModel(
            _catalogUiState.GetActiveEntries(),
            _catalogService);
        var window = new YibfIsTakibiEntryWindow(viewModel)
        {
            Owner = Application.Current?.Windows.OfType<Window>().FirstOrDefault(item => item.IsActive)
        };

        YibfIsTakibiEntry? result = null;
